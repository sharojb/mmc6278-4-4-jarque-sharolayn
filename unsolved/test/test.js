...
const files = readdirSync(path.join(__dirname, '../queries'))
      .filter(fileName => !fileName.match(/(update|insert|delete)/i))
...
describe('Music DB SQL Query Practice', () => {
  describe('Database Schema', () => {
    it('should have artists, albums, and songs table', async () => {
      ...
      expect(result.map(x => x.Tables_in_music_db)).to.have.members(['artists', 'songs', 'albums'])
    })
  ...
  describe('Other Queries', () => {
    beforeEach(async () => {
      ...
      execSync(`docker exec -d mysql-sandbox mysql -u root -e "source schema.sql"`)
      execSync(`docker exec -d mysql-sandbox mysql -u root -e "source seed.sql"`)
    ...
    it('06-insert.sql should insert row into songs table', async () => {
      ...
      const [result] = await db.query('SELECT * FROM songs WHERE name=?', ['Endless, Nameless'])
    ...
    it('07-update.sql should update row in albums table', async () => {
      ...
      await db.query(queryText)
      const [result] = await db.query('SELECT * FROM albums WHERE name=?', ['Metallica'])
    ...
    it('11-delete.sql should delete Justin Bieber', async () => {
      ...
      await db.query(queryText)
      const [result] = await db.query('SELECT * FROM artists WHERE name=?', ['Justin Bieber'])
      expect(result.length).to.eq(0)
      const [result2] = await db.query('SELECT * FROM artists WHERE name=?', ['Metallica'])
